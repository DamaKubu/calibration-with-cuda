...
int main(int argc, char** argv) {
    ...
    // Flatten all captured corners into one correspondence set.
    std::vector<cv::Point2f> allPts1;
    std::vector<cv::Point2f> allPts2;
    allPts1.reserve(static_cast<size_t>(captured) * static_cast<size_t>(patternSize.area()));
    allPts2.reserve(static_cast<size_t>(captured) * static_cast<size_t>(patternSize.area()));
    for (int i = 0; i < captured; i++) {
        allPts1.insert(allPts1.end(), imagePoints1[i].begin(), imagePoints1[i].end());
        allPts2.insert(allPts2.end(), imagePoints2[i].begin(), imagePoints2[i].end());
    }

    // Optional: Fundamental matrix in pixel coordinates (debug/inspection)
    cv::Mat fMask;
    F = cv::findFundamentalMat(allPts1, allPts2, cv::FM_RANSAC, 1.0, 0.99, fMask);
    std::cout << "Fundamental Matrix (pixel coords):\n" << F << "\n";

    // Normalize points (handles different intrinsics; also uses distortion if provided)
    std::vector<cv::Point2f> norm1, norm2;
    cv::undistortPoints(allPts1, norm1, K1, D1);
    cv::undistortPoints(allPts2, norm2, K2, D2);

    // Essential matrix in normalized camera coordinates
    cv::Mat eMask;
    E = cv::findEssentialMat(norm1, norm2, 1.0, cv::Point2d(0, 0), cv::RANSAC, 0.999, 1.0, eMask);
    std::cout << "Essential Matrix (normalized coords):\n" << E << "\n";

    // Recover pose (correct overload: R,t + focal/pp for normalized points)
    cv::recoverPose(E, norm1, norm2, R, T, 1.0, cv::Point2d(0, 0), eMask);
