...
int main(int argc, char** argv) {
    ...
    std::cout << "Controls: [Space]=save (needs FOUND)  [S]=save raw  [A]=toggle auto  [R]=rotate  [F]=flip  [Esc]=quit\n\n";
    ...
    while (saved < targetCount) {
        ...
        // Debug helper: save raw frame even if chessboard isn't detected.
        if (key == 's' || key == 'S') {
            const fs::path outPath = fs::path(outDir) / ("raw_" + cv::format("%04d", rawSaved) + ".png");
        ...
        if (manualSave && !found) {
            std::cout << "Skip: chessboard NOT found (check --pattern, lighting, and keep board fully visible)\n";
        }

        if ((manualSave || autoSave) && found) {
            if (blur < minBlur) {
                const bool shouldLog = manualSave || (std::chrono::duration_cast<std::chrono::milliseconds>(now - lastAutoSkipLog).count() >= 1500);
                if (shouldLog) {
                    std::cout << "Skip: blur " << blur << " < " << minBlur << (autoSave ? " (auto)" : "") << "\n";
                    lastAutoSkipLog = now;
                }
                continue;
            }
            if (!cornersChangedEnough(prevCorners, corners, minShiftPx)) {
                const bool shouldLog = manualSave || (std::chrono::duration_cast<std::chrono::milliseconds>(now - lastAutoSkipLog).count() >= 1500);
                if (shouldLog) {
                    std::cout << "Skip: pose too similar (min_shift_px=" << minShiftPx << ")" << (autoSave ? " (auto)" : "") << "\n";
                    lastAutoSkipLog = now;
                }
                continue;
            }
